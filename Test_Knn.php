...
$fp = fopen('Knn.csv', 'w');
    ...
    //แปลง CSV เป็น arff
    $cmd = 'java -classpath "weka.jar" weka.core.converters.CSVLoader Knn.csv > Knn_Unseen.arff ';
    exec($cmd,$output);

    //แปลงข้อมูลให้เป็น Nominal
    $cmd2 = 'java -classpath "weka.jar" weka.filters.unsupervised.attribute.NumericToNominal -i Knn_Unseen.arff -o Knn_Unseen_Nomial.arff ';
    exec($cmd2,$output);

    //ทำ Model กำหนด Folds = 15 , K = 3
    $cmd3 = 'java -classpath "weka.jar" weka.classifiers.lazy.IBk -x 15 -K 3 -t "breast-cancer.arff" -d Knn.model';
    exec($cmd3,$output);

    //โหลด Model มาใช้
    $cmd4 = 'java -classpath "weka.jar" weka.classifiers.lazy.IBk -l "Knn.model" -T Knn_Unseen_Nomial.arff -p 10';
    exec($cmd4,$output1);
